...
architecture rtl of rebotes is
    -- Señal equivalente a "col_x_reg" de la Figura 3
...
begin
    ...
    begin
        if nRST = '0' then 
            col_reg      <= (others => '1');
            columnas_out <= (others => '1');
            
        elsif clk'event and clk = '1' then 
            -- Registro de las columnas en tiempo real (evita metaestabilidad)
            col_reg <= columnas_in;
            
            -- Filtro: Solo se actualiza la salida cuando llega el tic (ej: 5ms)
            if tic = '1' then
                columnas_out <= col_reg;
            end if;
        end if;
    end process;
